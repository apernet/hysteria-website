...
«Gecko» — это уровень обфускации, оборачивающий «Salamander». Его цель — скрыть характерную форму QUIC-датаграмм рукопожатия, разбивая их на несколько фрагментов случайного размера со случайным заполнением.

Отправитель проверяет первый байт каждого исходящего QUIC-пакета:

- **Пакеты с коротким заголовком** (старший бит `0x80` сброшен): отправляются как есть через Salamander.
- **Пакеты с длинным заголовком** (старший бит `0x80` установлен): разбиваются на N фрагментов, где `2 <= N <= 8`. Каждый фрагмент оборачивается в Gecko-кадр и отправляется через Salamander как независимая датаграмма.

Gecko-кадр имеет следующий формат (до шифрования Salamander):

```
[1 байт]  флаги         — ДОЛЖЕН быть 0x80 (маркер фрагмента)
[1 байт]  msgID         — идентифицирует исходный пакет; произвольный со стороны отправителя
[1 байт]  chunkIdx:4 | totalChunks:4
[2 байта] padLen        — uint16, big-endian
[padLen байт] padding   — случайные байты, игнорируются при приёме
[байты]   chunk         — кусок исходного пакета, относящийся к этому фрагменту
```

`chunkIdx` ДОЛЖЕН быть меньше `totalChunks`, а `totalChunks` ДОЛЖЕН быть в диапазоне `[2, 8]`. Фрагменты одного исходного пакета имеют одинаковые `msgID` и `totalChunks`. Конкатенация кусков в порядке `chunkIdx` точно воспроизводит исходный QUIC-пакет.

Получатель:

1. Деобфусцирует каждую датаграмму через Salamander. Если старший бит первого байта сброшен, датаграмма рассматривается как QUIC-пакет с коротким заголовком и передаётся без изменений.
2. Иначе разбирает Gecko-кадр. Некорректные кадры ДОЛЖНЫ быть отброшены.
3. Буферизует куски по ключу `(адрес источника, msgID)` до прихода всех `totalChunks`, после чего передаёт собранный пакет в QUIC.

Получателям СЛЕДУЕТ ограничивать состояние сборки (лимит на источник, общий лимит и TTL) и отбрасывать дубликаты, а также кадры с несовпадающим `totalChunks`. Конкретные ограничения определяются реализацией.

Диапазон заполнения, распределение количества фрагментов и выбор последовательности `msgID` — это решения отправителя и не согласовываются.
