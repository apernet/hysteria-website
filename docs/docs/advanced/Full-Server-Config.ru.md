...
Поле `listen` также поддерживает диапазоны портов для [смены портов](Port-Hopping.md):

```yaml
listen: :20000-50000 # (1)!
```

1. Сервер будет слушать на первом порту диапазона и автоматически настроит правила файрвола (nftables или iptables) для перенаправления трафика с остальных портов на первый. Правила автоматически удаляются при остановке сервера.

> **ПРИМЕЧАНИЕ:** Прослушивание диапазона портов на стороне сервера поддерживается только в Linux. Требуется наличие `nft` (nftables) или `iptables`/`ip6tables`. Серверу могут потребоваться соответствующие привилегии (например, root или `CAP_NET_ADMIN`) для изменения правил файрвола.

Поле `listen` также принимает URI [Hysteria Realms](Realms.md), чтобы запустить сервер в P2P-режиме за NAT:
...
1. STUN-серверы, используемые для обнаружения публичных UDP-адресов сервера. По умолчанию используется небольшой встроенный список.
2. Таймаут запроса к одному STUN-серверу.
3. Максимальное время ожидания успешного UDP-пробивания для одной попытки подключения.
4. Как часто отправлять heartbeat на rendezvous, чтобы держать realm зарегистрированным. Сессия истекает, если rendezvous вовремя не получает heartbeat.
5. Установите в `true` только для пропуска проверки TLS у самоподписанного сервера rendezvous (разработка).
6. Ограничить обход NAT одним семейством IP: `dual` (по умолчанию, и IPv4, и IPv6), `v4` (только IPv4) или `v6` (только IPv6).
7. Включить проброс портов UPnP/NAT-PMP на вашем шлюзе для повышения успешности пробивания. Сбои некритичны — выполняется откат к адресам, обнаруженным через STUN.
8. Таймаут на одну операцию при обращении к шлюзу.
9. Срок аренды проброса порта; продлевается автоматически до истечения.
...
При желании вы можете включить [Encrypted Client Hello](ECH.md), чтобы скрыть TLS SNI от промежуточных устройств.
...
1. Путь к файлу ключа ECH, сгенерированному с помощью `sing-box generate ech-keypair <public_name>`. При запуске сервер выводит в лог config list, который клиентам нужно указать в их `tls.ech`. Подробности см. в [ECH](ECH.md).
...
Маскировка соединения под TCP с помощью [mimic](https://github.com/hack3ric/mimic). Требует установленных mimic и его модуля ядра, а также запуска Hysteria с правами root. **У всех клиентов должна быть та же настройка**, иначе они вообще не смогут подключиться.
...
1. Включить ли mimic. **Должно совпадать у всех клиентов.**
2. Интерфейс, к которому присоединяться. Если не указан, используется тот, что смотрит в интернет.
...
Включение mimic на уже работающем сервере — ломающее изменение: клиенты без него больше не смогут подключиться. Кроме того, оно отключает сегментацию UDP (GSO), что снижает пропускную способность. Подробнее см. [Mimic](Mimic.md).
...
`ignoreClientBandwidth` — это специальная опция, которая при включении заставляет сервер игнорировать любые подсказки о полосе пропускания от клиентов и использовать настроенный не-Brutal контроллер. Это фактически переопределяет любые значения полосы пропускания, установленные клиентами в обоих направлениях.
...
Эта секция выбирает контроллер перегрузки и профиль его поведения. Она применяется только тогда, когда для данного направления не выбран Brutal (см. раздел `bandwidth` выше).
...
Секция `congestion` является локальной для каждой стороны и не согласовывается по протоколу. В большинстве сценариев рекомендуется указывать одинаковые настройки перегрузки на клиенте и сервере.

### Процесс согласования полосы пропускания

Следующая диаграмма показывает, как определяется, будет ли данное направление использовать Brutal или не-Brutal контроллер. Когда Brutal не выбран, соответствующая сторона использует локальное значение `congestion.type` (`bbr` по умолчанию или `reno`, если он выбран).

```mermaid
graph TD;
    ICB{{"Настроено ли на сервере ignoreClientBandwidth: true?"}} -- "Да" --> NonBrutal[/"Используется настроенный не-Brutal контроллер"/];
    ICB -- "Нет" --> C_has_BW;
    C_has_BW{{"Настроена ли на клиенте bandwidth?"}} -- "Нет" --> NonBrutal;
    C_has_BW -- "Да" --> Brutal;
...
graph TD;
    ...
    C_has_BW{{"Настроена ли на клиенте bandwidth?"}} -- "Нет" --> NonBrutal[/"Используется настроенный не-Brutal контроллер"/];
...
В настоящее время в Hysteria есть 3 режима управления перегрузкой:
...
**Reno:** Это контроллер перегрузки по умолчанию из `quic-go`. Он проще и обычно более консервативен, чем BBR. Выбирается с помощью `congestion.type: reno`.
...
Алгоритмы управления перегрузкой контролируют отправку данных. С точки зрения клиента, если пользователь не указал значение полосы пропускания для скачивания (но указал для загрузки), сервер Hysteria будет отправлять данные клиенту, используя свой локально настроенный не-Brutal контроллер, а клиент будет отправлять данные на сервер с использованием Brutal, и наоборот. Клиент может указать оба значения, тогда оба направления будут использовать Brutal, или не указывать ни одного — тогда оба направления будут использовать настроенный не-Brutal контроллер (по умолчанию BBR).

Особый случай, упомянутый выше, — когда на сервере включено `ignoreClientBandwidth`; в этом случае обе стороны игнорируют подсказки о полосе пропускания и используют свои локальные не-Brutal настройки.

**Ограничение полосы пропускания сервера в настоящее время применяется только к Brutal. Оно не влияет на BBR или Reno.**
...
8. Необязательно. Устанавливать ли заголовки `X-Forwarded-For`, `X-Forwarded-Host` и `X-Forwarded-Proto` при проксировании запросов. По умолчанию отключено.
