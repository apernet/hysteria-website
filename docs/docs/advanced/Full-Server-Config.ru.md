...
3. Как часто заново выполнять STUN-обнаружение и обновлять адреса на сервере rendezvous.
4. Максимальное время ожидания успешного UDP-пробивания для одной попытки подключения.
5. Как часто отправлять heartbeat на rendezvous, чтобы держать realm зарегистрированным. Сессия истекает, если rendezvous вовремя не получает heartbeat.
6. Установите в `true` только для пропуска проверки TLS у самоподписанного сервера rendezvous (разработка).
