...
Если ограничение касается UDP в целом, а не конкретного порта, смена портов не поможет. [Mimic](Mimic.md) вместо этого маскирует соединение под TCP, но эти два способа нельзя использовать вместе.
