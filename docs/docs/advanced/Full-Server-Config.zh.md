...
当使用 HTTP 认证，客户端尝试连接时，服务器会向后端服务器发送一个带有以下 JSON 的 `POST` 请求：
...
出站规则用于定义连接应通过哪个“出口”。例如，与 ACL 结合使用时，可以实现除 Netflix 外的所有流量都直接使用服务器本地网络，而 Netflix 流量通过另一个 SOCKS5 代理连接。
