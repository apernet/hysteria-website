...
1. Сервер Hysteria регистрирует _realm_ (имя на ваш выбор) в сервисе рандеву, сообщая UDP-адреса, обнаруженные через STUN.
...
> **NOTE:** В v2.9.0 сертификаты, созданные `hysteria cert`, приводят к ошибке `tls: internal error` на клиенте. Это будет исправлено в следующем релизе. До тех пор добавьте `sniGuard: disable` в существующую секцию `tls` на **сервере** (сохранив `cert`, `key` и остальные поля):
>
> ```yaml
> tls:
>   cert: server.crt
...
|                                  | Публичный IP | Full cone | Restricted / Port-restricted | Sym (предсказуемый) | Sym (случайный) |
| -------------------------------- | ------------ | --------- | ---------------------------- | ------------------- | --------------- |
| **Публичный IP**                 | ✅           | ✅        | ✅                           | ✅                  | ✅              |
| **Full cone**                    | ✅           | ✅        | ✅                           | ✅                  | ✅              |
| **Restricted / Port-restricted** | ✅           | ✅        | ✅                           | ⚠️                  | ❌              |
| **Sym (предсказуемый)**          | ✅           | ✅        | ⚠️                           | ⚠️                  | ❌              |
| **Sym (случайный)**              | ✅           | ✅        | ❌                           | ❌                  | ❌              |

- ✅ Работает надёжно.
- ⚠️ Работает иногда — зависит от того, может ли Hysteria предсказать следующий порт симметричного NAT.
- ❌ Как правило, не работает.

Примечания:

- Матрица не зависит от направления: какая сторона «клиент», а какая «сервер», значения не имеет.
- Любая пара с **Публичным IP** или **Full cone** работает, потому что нерестриктивная сторона принимает первый пакет пира с любого порта, который выбрал его NAT.
- Случаи `Sym (предсказуемый)` опираются на то, что Hysteria видит несколько STUN-рефлексивных адресов в близком диапазоне портов. Стандартных 3 STUN-серверов обычно достаточно, чтобы закономерность стала видна; одного сервера эвристике уже не хватит.
- Любая пара с `Sym (случайный)`, кроме Публичного IP или Full cone, в принципе нерешаема. Это свойство UDP hole punching, а не ограничение Hysteria.

Если Realms у вас не работает, наиболее вероятная причина — симметричный NAT хотя бы с одной стороны. В этом случае возвращайтесь к серверу с публичным IP.
