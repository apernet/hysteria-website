...
[Encrypted Client Hello (ECH)](https://blog.cloudflare.com/announcing-encrypted-client-hello) is a TLS extension that encrypts the Client Hello message, including the Server Name Indication (SNI). This prevents passive network observers from determining which server a client is connecting to based on the SNI.

To use ECH, you need to generate an ECH key pair first:
...
This generates two files: `ech.key` for the server and `ech.config` for the client. The argument (`example.com` in this example) is the "outer SNI" - the public-facing server name that will be visible to observers instead of the real one.

To enable ECH on the server, add the following to your configuration:
...
1. The path to the ECH key file generated by `hysteria generate ech-keypair`.

> **NOTE:** Active probing could theoretically reveal ECH usage, though as of now no major censors have deployed such detection methods.
