...
1. Hysteria **服务端**通过 STUN 发现自己的 UDP 公网地址，把这些地址连同你选择的 _realm_（一个名字）注册到牵线服务。
...
|                         | 公网 IP | 全锥 | 限制锥 / 端口限制锥 | 对称（可预测） | 对称（随机） |
| ----------------------- | ------- | ---- | ------------------- | -------------- | ------------ |
| **公网 IP**             | ✅      | ✅   | ✅                  | ✅             | ✅           |
| **全锥**                | ✅      | ✅   | ✅                  | ✅             | ✅           |
| **限制锥 / 端口限制锥** | ✅      | ✅   | ✅                  | ⚠️             | ❌           |
...
- ✅ 通常可用。
- ⚠️ 有时可用，取决于 Hysteria 能否成功预测对称 NAT 端的端口。
- ❌ 通常失败。

说明：

- 矩阵与方向无关：哪一端是「客户端」、哪一端是「服务端」并不影响结果。
- 任一端为**公网 IP**或**全锥**时都能成功，因为不受限的一端会接受对端 NAT 任何端口发来的数据包。
- `对称（可预测）` 的情况依赖于 Hysteria 观察到多个落在相近端口范围内的 STUN 反射地址。默认的 3 个 STUN 服务器通常足以暴露这种规律；只用单个 STUN 服务器会导致无法获得足够的信息来预测端口。
- 任一端为 `对称（随机）`、另一端又不是公网 IP 或全锥时，无法打通。这是 UDP 打洞本身的限制，并非 Hysteria 实现问题。

如果 Realms 在你的网络上无法工作，最可能的原因是至少一侧是对称 NAT。请退而使用有公网 IP 的服务器。
