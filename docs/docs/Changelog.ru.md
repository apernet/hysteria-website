...
- Исправлено медленное переподключение после простоя или сна клиента, особенно заметное на мобильных устройствах. Сервер теперь отправляет QUIC stateless reset, поэтому клиент с устаревшим соединением переподключается сразу, а не ждёт истечения своего тайм-аута простоя
- Исправлен отказ [mimic](advanced/Mimic.md) запускаться, когда его необязательный модуль ядра не загружен
- Различные обновления зависимостей

## 2.12.0

- Добавлена интеграция с [mimic](advanced/Mimic.md), которая маскирует соединение под TCP для сетей, ограничивающих UDP (только Linux, mimic нужно установить отдельно)
- Повышена точность имитации QUIC-отпечатка Chrome
- Исправлена паника BBR на путях с малым MTU, появившаяся в 2.11.0 из-за имитации отпечатка Chrome
- Различные обновления зависимостей
...
- Добавлен параметр `bandwidth.disableLossCompensation` для отключения механизма компенсации потерь в управлении перегрузкой Brutal, что в некоторых случаях может улучшить стабильность
- Различные обновления зависимостей и мелкие исправления ошибок

## 2.9.3

- Добавлена поддержка проброса портов UPnP/NAT-PMP в [Hysteria Realms](advanced/Realms.md) для упрощения обхода NAT
- Добавлен параметр `ipMode` в Hysteria Realms для ограничения подключений только IPv4 или только IPv6
- Обновлён quic-go до v0.60.0
- Исправлено повреждение `+` в учётных данных аутентификации (замена на пробел) при разборе ссылок `hysteria2://`
- Исправлено отклонение HTTP-прокси действительных учётных данных Basic-аутентификации
- Исправлено использование `host:port` вместо имени хоста в качестве TLS SNI в HTTPS-прокси outbound
- Исправлена гонка данных в мультиплексоре слушателей HTTP/SOCKS5 на клиенте
- Исправлен сбой перенаправления nftables при привязке сервера к конкретному адресу IPv6
- DNS-резолвер теперь ограничивает длину цепочки CNAME для предотвращения неограниченной рекурсии

## 2.9.2

> Этот релиз содержит важные исправления безопасности, и мы настоятельно рекомендуем всем обновиться.

- Добавлена [обфускация Gecko](advanced/Full-Server-Config.md#obfuscation): новый экспериментальный слой обфускации, который фрагментирует QUIC-пакеты рукопожатия
- Исправлена проблема безопасности, при которой UDP-пакеты могли обходить ACL
- Исправлен возможный OOM сервера из-за неполных или слишком больших HTTP-запросов при включённом sniff
- Исправлен обход ACL через завершающую точку в доменном имени (например, `example.com.`)
- Исправлена некорректная обработка адреса назначения в SOCKS5 UDP outbound
- Исправлено определение префикса `https://` в DoH-резолвере
- Outbound-правила теперь отклоняют недопустимые значения порта
- Незначительные улучшения производительности

## 2.9.1

- Исправлена проблема, при которой клиент не мог подключиться к серверу за симметричным NAT, что повышает успешность hole punching
- Команда `hysteria cert` теперь добавляет `sniGuard: disable` в пример серверной конфигурации, чтобы самоподписанные сертификаты работали с Realms «из коробки»

## 2.9.0

- Добавлены [Hysteria Realms](advanced/Realms.md): нет публичного IP? Не беда. Пробивает ваш NAT, чтобы вы могли держать сервер Hysteria дома, в мобильной сети или даже в кафе. Клиенты подключаются напрямую, peer-to-peer. Без проброса портов, без релея.

## 2.8.2

> Этот релиз содержит важные исправления безопасности, и мы настоятельно рекомендуем всем обновиться.

- Исправлена проблема безопасности, при которой при включённом sniff злоумышленник мог создать вредоносные QUIC-пакеты и вызвать OOM-сбой сервера
- Исправлена проблема совместимости с некоторыми старыми версиями nftables при включённой смене портов на сервере
- Исправлена потенциальная проблема потокобезопасности в salamander obfs
- **Важно: из-за изменений в обработке некоторых параметров QUIC-рукопожатия UDP-переадресация не будет работать, когда клиенты v2.8.2 подключаются к старым версиям сервера (TCP не затронут). Новые серверы полностью совместимы как с новыми, так и со старыми клиентами. Настоятельно рекомендуется обновить и серверы, и клиенты.**
