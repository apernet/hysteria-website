...
Hysteria может использовать Brutal, BBR или Reno. **Секция `bandwidth` по-прежнему определяет, будет ли в этом направлении использоваться Brutal.** Если удалить секцию `bandwidth`, клиент перейдёт на не-Brutal контроллер, настроенный в `congestion`; по умолчанию это BBR с профилем `standard`.

Подробнее см. [Процесс согласования полосы пропускания](../advanced/Full-Server-Config.md#bandwidth-negotiation-process), [Детали управления перегрузкой](../advanced/Full-Server-Config.md#congestion-control-details) и [Полную конфигурацию клиента](../advanced/Full-Client-Config.md).
