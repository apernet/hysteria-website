...
**请将 `up` 和 `down` 的带宽值设置为你当前网络环境的理论最大速度。为了达到最佳性能，请务必设置准确。** 如果实在不知道上下行中一侧或者两侧的带宽，可以只设置一侧的带宽，移除另一侧，或者两侧都移除。这些带宽设置所控制的行为在 [完整客户端配置 - 带宽部分](../advanced/Full-Client-Config.md#_5) 中有描述。
