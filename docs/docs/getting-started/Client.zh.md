...
**请将 `up` 和 `down` 的带宽值设置为你当前网络的最大速率。** 强烈建议设置这些值，但不是必填的。如果你对上下行一侧或者两侧的带宽不确定，你可以只设置一侧，或者完全去掉这个带宽部分。这些带宽设置所控制的行为在 [完整客户端配置 - 带宽部分](../advanced/Full-Client-Config.md#_5) 中有描述。
