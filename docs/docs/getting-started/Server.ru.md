...
> **Нет публичного IP?** Вы всё равно можете держать сервер Hysteria из дома, мобильной сети или любой среды за NAT с помощью [Hysteria Realms](../advanced/Realms.md). Realms использует UDP-пробивание, чтобы клиенты подключались напрямую — без проброса портов и без переадресации трафика через посредника. Остальное руководство применимо без изменений — меняется только поле `listen`.
